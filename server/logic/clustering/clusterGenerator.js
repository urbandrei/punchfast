...
async function createRouteFromCluster(cluster, cuisine, geoCache, mstEdges) {
    try {
        return await sequelize.transaction(async (t) => {
            ...
            // Mark stores as in_route
            await Store.update(
                { in_route: true },
                { where: { id: { [Op.in]: orderedStoreIds } }, transaction: t }
            );
...
async function generateRoutesFromNewStores(boundingBox, remainderBoxes) {
    ...
    try {
        // fetchStoresInArea now filters by in_route: false, so it returns only unclustered stores
        const unclusteredStores = await fetchStoresInArea(boundingBox);

        if (unclusteredStores.length === 0) {
            console.log('[Clustering] No unclustered stores found in search area');
            return {
                routesCreated: 0,
                storesProcessed: 0,
                routes: []
