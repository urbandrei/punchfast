...
exports.getUserActiveRoutes = async (req, res) => {
    const { userId } = req.params;
    ...
    try {
        ...
        return res.status(200).json({ count: result.length, activeRoutes: result });
    } catch (error) {
        console.error('Error fetching user active routes:', error);
        return res.status(500).json({ message: 'Server error', error: error.message });
    }
};
